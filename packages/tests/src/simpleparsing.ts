...
async function testHTMLParsing(htmlContent: string): Promise<void> {
    ...
    const headerCellCollection:NodeListOf<HTMLTableCellElement> = table.querySelectorAll(':scope tr :has(th)');
            //table.querySelectorAll('th');

        
    if (!headerCellCollection) throw new Error("No header cells found");
    
    let colSpanSum:number = 1;

    headerCellCollection.forEach((headerCell:HTMLTableCellElement,index:number) => {
        console.log(`header cells span: ${headerCell.colSpan}`);

        colSpanSum += Number(headerCell.getAttribute('colspan')) ?? 1;
        //if (headerCell.getAttribute('colspan') && headerCell.)
    });

    console.log(`total header cells span: ${colSpanSum}`);
...
async function main() {
    console.log('..................................................');
    const htmlDoc: string = await fs.promises.readFile(path.join('./static/AMDR2.htm'), 'utf-8');
